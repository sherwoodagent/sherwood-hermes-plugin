...
def test_check_cli_installed_ok():
    fake_run = MagicMock(return_value=MagicMock(returncode=0, stdout="0.68.0"))
    with patch("subprocess.run", fake_run):
        ok, version = check_cli_installed("sherwood")
    assert ok is True
    assert version == "0.68.0"
...
def test_check_cli_installed_version_too_old():
    fake_run = MagicMock(return_value=MagicMock(returncode=0, stdout="0.67.9"))
    with patch("subprocess.run", fake_run):
        ok, version = check_cli_installed("sherwood", min_version="0.68.0")
    assert ok is False
    assert version == "0.67.9"
