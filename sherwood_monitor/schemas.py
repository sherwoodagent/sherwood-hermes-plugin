...
START = {
    ...
    "description": (
        "Start monitoring a Sherwood fund. Spawns a streaming subprocess "
        "that forwards on-chain events and XMTP messages into this conversation. "
        "Use this when the user asks to watch a new fund or after adding "
        "one to the config."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "subdomain": {
                "type": "string",
                "description": "Sherwood fund subdomain, e.g. 'alpha-fund'",
            }
        },
        "required": ["subdomain"],
    },
...
STOP = {
    ...
    "description": (
        "Stop monitoring a Sherwood fund. Terminates the streaming "
...
STATUS = {
    ...
    "description": (
        "Get the status of all monitored funds: pid, uptime, events seen, "
        "last event time, and recent stderr. Use to answer 'is my fund "
...
EXPOSURE = {
    ...
    "description": (
        "Aggregate exposure across all configured funds. Returns total "
...
CRON_TICK = {
    ...
    "description": (
        "Autonomous tick: catch new interesting events for a fund since "
